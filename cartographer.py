...
class Cartographer:
    ...
    def _shift(
        ...
        """
        # validate the inputs
        if not isinstance(point, ParameterVector):
            raise TypeError(f"Expected 'point' type ParameterVector, got {type(point)}")
        if not isinstance(direction, ParameterVector):
    ...
    def _measure_loss(
        ...
        Args:
            model (ParameterVector): The model to evaluate.
        ...
        model.eval()
        with torch.no_grad(): # TODO: Disable gradients globally
